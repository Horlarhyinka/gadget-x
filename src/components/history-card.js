...
const {status, items, createdAt} = history
    ...
    const itemsList =()=> items.length >= 1?items.map(({name, description, price, _id, quantity})=><div key={_id} className="field"><label>{name + " ("+ quantity +")"}</label><p>₦{numberToPrice((price*quantity).toFixed(2))}</p></div>):<p>null</p>;
    ...
    return ( <div className="history-card">
        ...
        <div className="field"><label>Total</label><p>₦{numberToPrice(items.reduce((acc,{price,quantity})=>acc += price*quantity,0))}</p></div>
