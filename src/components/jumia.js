...
const JumiaProducts = ({keyword, displayMessage}) => {
    ...
    const queryUrl = process.env.REACT_APP_API_BASE_URL + "products/jumia"

    useEffect(()=>{
            axios.get(queryUrl+"?key="+keyword)
            .then(res=>{
                displayMessage({message:"external datas fetched successfully",status:"info"})
