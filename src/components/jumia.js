...
let index = 0;
const listAll = data && data.map(({preview,discount,price})=>{
    index++
    return preview&&discount&&price && (<div key={index} className="jumia-product-card product-card">
                                            <img src={jumia_image_url} alt="from jumia" />
                                            <div>
    ...
    return !data?<h1>No jumia data found</h1>:(<div className="jumia">
        <h4>From Jumia</h4>
        {listAll}
    </div>)
}
